...
// 获取已解锁的用户
import { getAccountList } from '~/api/subscribed';
const subscribeUserList = ref([]);
const subscribeList = useState('subscribeUserList', () => subscribeUserList.value || []);
watchEffect(async () => {
  if (userInfo.value) {
    const { data: subscribeUserData } = await getAccountList();
    subscribeUserList.value = subscribeUserData.value;
    subscribeList.value = subscribeUserList.value;
  }
});
// 已订阅的总未读数量
const subscribeNum = useState('subscribeNum', () => {
  return subscribeList.value.reduce((prev, cur) => {
    return prev + (cur.unread_num || 0);
  }, 0);
});
const menuList = computed(() => {
  return [
    ...
    {
      name: 'subscribed',
      label: t('subscribed.title'),
      path: `/subscribed`,
      icon: 'icon-park-outline:unlock',
      icon_active: 'icon-park-solid:unlock',
      badge: subscribeNum.value,
      isShow: () => {
        return subscribeUserList.value?.length;
      },
      mobileShow: () => false,
    },
    {
...
const showRightPanel = computed(() => {
  const noShowRightPanelRoutes = ['message', 'subscribed'];
    ...
    <div class="mobile-footer" :class="{ hide: !showMobileFooter }">
      <div class="mobile-footer-top">
        ...
        <div class="mobile-handle-backtop" v-show="showBackTop">
          <a-back-top :target-container="htmlDom" style="left: 20px; bottom: 30px" :visible-height="200" />
        </div>
        <!-- 手机端底部按钮--发推 -->
